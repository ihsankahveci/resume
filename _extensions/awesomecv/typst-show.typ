// Set document metadata before any content (required by Typst 0.11)
$if(title)$
#set document(
  title: "$title-meta$$if(title-meta)$$else$$title$$endif$",
$if(author)$
...
$if(author)$
  author: (
    ...
    address: ($for(author.address)$unescape_text("$it$")$sep$, $endfor$),
